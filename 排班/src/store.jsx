...
function StoreProvider({ children }) {
  ...
  useEffect(() => { saveState('duty_group_post_pointers', groupPostPointers); }, [groupPostPointers]);

  // 分组模式主副班轮换：0=A主B副，1=B主A副，每次生成后翻转
  const [groupRotation, setGroupRotation] = useState(() => {
    return loadState('duty_group_rotation', 0);
  });
  useEffect(() => { saveState('duty_group_rotation', groupRotation); }, [groupRotation]);

  const [gatePointer, setGatePointer] = useState(() => {
    return loadState('duty_gate_pointer', 0);
  });
  useEffect(() => { saveState('duty_gate_pointer', gatePointer); }, [gatePointer]);

  const [afterSchoolPointer, setAfterSchoolPointer] = useState(() => {
    return loadState('duty_afterschool_pointer', 0);
  });
  useEffect(() => { saveState('duty_afterschool_pointer', afterSchoolPointer); }, [afterSchoolPointer]);

  const [scheduleHistory, setScheduleHistory] = useState(() => {
    return loadState('duty_schedule_history', []);
  });
  useEffect(() => { saveState('duty_schedule_history', scheduleHistory); }, [scheduleHistory]);
